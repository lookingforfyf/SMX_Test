...
void SM3_self_test(void)
{
	...
	printf("%-6s: %s\n\n", "msg_1", msg_1);
	printf("%-6s: ", "hash_1"); dump_byte(hash1, 32);
	printf("%-6s: %s\n\n", "msg_2", msg_2);
	printf("%-6s: ", "hash_2"); dump_byte(hash2, 32);

	SM3((unsigned char*)msg_1, strlen(msg_1), hash);
	printf("sm3(msg_1): "); dump_byte(hash, sizeof(hash));

	if(0 != memcmp(hash, hash1, 32)) printf("sm3 hash test error\n\n");

	SM3((unsigned char*)msg_2, strlen(msg_2), hash);
	printf("sm3(msg_2): "); dump_byte(hash, sizeof(hash));
